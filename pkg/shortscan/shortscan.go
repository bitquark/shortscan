...
func Scan(url string, hc *http.Client, st *httpStats, ac attackConfig, mk markers) {
	...
	// Bail if we're just running a vuln check
	if args.IsVuln {
		os.Exit(0)
	}
...
func Run() {
	...
	for s.Scan() {
		...
		if ac.wordlist.isRainbow {
			...
			ac.wordlist.wordlist = append(ac.wordlist.wordlist, wordlistRecord{c[0], f, e, f83, e83})
